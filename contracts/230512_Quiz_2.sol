...
contract PASSWORD {
    ...
    Member[] members; 
    mapping(string => bytes32) password_Bytes32;

//value인 bytes32는 ID와 PW를 같이 넣은 후 나온 결과값으로 설정하기
// * 로그인 기능 - ID, PW를 넣으면 로그인 여부를 알려주는 기능
    function logIn(string memory _id, uint _password) public view returns(bool){
        if (password_Bytes32[_id] = bytes32(_password)) {
            return true;
        } else {
            return false;
        }
...
// * 회원가입 기능 - 새롭게 회원가입할 수 있는 기능
    function registerNew(string memory _id, uint _password) public {
    password_Bytes32[_id] = bytes32(_password);
    }
}